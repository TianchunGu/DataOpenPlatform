...
string& deleterchr(string& str, const int cc) {
  auto pos =
      str.find_last_not_of(cc);  // 从字符串的右边查找第一个不是cc的字符的位置。
...
bool replacestr(string& str,
  ...
  while (true) {
    if (bloop == true)
      ...
      ppos =
          str.find(str1, pstart);  // 从上次执行替换的位置后开始查找子串str1。
...
bool clogfile::open(const string& filename,
                    const ios::openmode mode,
                    const bool bbackup,
                    const bool benbuffer) {
  // 如果日志文件是打开的状态，先关闭它。
  if (fout.is_open())
    fout.close();
...
bool clogfile::backup() {
  ...
  if (fout.tellp() > m_maxsize * 1024 * 1024) {
    ...
    rename(m_filename.c_str(),
           bak_filename.c_str());  // 把当前日志文件改名为备份日志文件。

    fout.open(m_filename, m_mode);  // 重新打开当前日志文件。

    if (m_enbuffer == false)
      fout << unitbuf;  // 判断是否启动文件缓冲区。
...
bool cofile::open(const string& filename,
                  const bool btmp,
                  const ios::openmode mode,
                  const bool benbuffer) {
  // 如果文件是打开的状态，先关闭它。
...
bool cofile::closeandrename() {
  ...
  if (m_filenametmp.empty() == false)
    if (rename(m_filenametmp.c_str(), m_filename.c_str()) != 0)
      return false;
...
int filesize(const string& filename) {
  ...
  // 获取文件信息，存放在结构体中。
  if (stat(filename.c_str(), &st_filestat) < 0)
    return -1;
...
bool setmtime(const string& filename, const string& mtime) {
  ...
  if (utime(filename.c_str(), &stutimbuf) != 0)
    return false;
...
bool cdir::opendir(const string& dirname,
                   const string& rules,
                   const int maxfiles,
...
bool cdir::_opendir(const string& dirname,
                    const string& rules,
                    const int maxfiles,
                    const bool bandchild) {
  DIR* dir;  // 目录指针。
  ...
  while ((stdir = ::readdir(dir)) != 0)  // readdir与库函数重名，需要加::
  {
    ...
    if (stdir->d_type == 4) {
      ...
      {
        if (_opendir(strffilename, rules, maxfiles, bandchild) ==
            false)  // 递归调用_opendir函数。
        {
          closedir(dir);
          return false;
        }
...
bool cdir::readdir() {
  ...
  m_mtime =
      timetostr1(st_filestat.st_mtime, m_fmt);  // 文件最后一次被修改的时间。
  m_ctime = timetostr1(st_filestat.st_ctime, m_fmt);  // 文件生成的时间。
  m_atime =
      timetostr1(st_filestat.st_atime, m_fmt);  // 文件最后一次被访问的时间。
...
bool renamefile(const string& srcfilename, const string& dstfilename) {
  // 如果原文件不存在，直接返回失败。
  if (access(srcfilename.c_str(), R_OK) != 0)
    return false;

  // 创建目标文件的目录。
  if (newdir(dstfilename, true) == false)
    return false;
...
bool ctcpserver::read(void* buffer,
                      const int ibuflen,
                      const int itimeout)  // 接收二进制数据。
{
  if (m_connfd == -1)
    return false;
...
bool ctcpclient::read(void* buffer,
                      const int ibuflen,
                      const int itimeout)  // 接收二进制数据。
{
  if (m_connfd == -1)
...
bool ctcpserver::write(const void* buffer,
                       const int ibuflen)  // 发送二进制数据。
{
  if (m_connfd == -1)
    return false;
             ...
             const int itimeout)  // 接收二进制数据。
{
  ...
  // 读取报文内容。
  if (readn(sockfd, (char*)buffer, ibuflen) == false)
    return false;
             ...
             const int itimeout)  // 接收文本数据。
{
  ...
  // 先读取报文长度，4个字节。
  if (readn(sockfd, (char*)&buflen, 4) == false)
    return false;

  buffer.resize(buflen);  // 设置buffer的大小。

  // 再读取报文内容。
  if (readn(sockfd, &buffer[0], buflen) == false)
    return false;
              ...
              const int ibuflen)  // 发送二进制数据。
{
  ...
  if (writen(sockfd, (char*)buffer, ibuflen) == false)
    return false;
...
bool tcpwrite(const int sockfd, const string& buffer)  // 发送文本数据。
{
  ...
  // 先发送报头。
  if (writen(sockfd, (char*)&buflen, 4) == false)
    return false;

  // 再发送报文体。
  if (writen(sockfd, buffer.c_str(), buflen) == false)
    return false;
...
bool copyfile(const string& srcfilename, const string& dstfilename) {
  ...
  if (ofile.open(dstfilename, ios::out | ios::binary) == false)
    return false;
...
// 把当前进程的信息加入共享内存进程组中。
bool cpactive::addpinfo(const int timeout,
                        const string& pname,
                        clogfile* logfile) {
  if (m_pos != -1)
    return true;
  ...
  */
  st_procinfo stprocinfo(getpid(), pname.c_str(), timeout,
                         time(0));  // 当前进程心跳信息的结构体。
